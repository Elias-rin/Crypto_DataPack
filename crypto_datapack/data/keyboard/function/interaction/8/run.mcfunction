...
#データ編集
  ...
  #入力文字をstorageに格納
  data modify storage keyboard: argument.input set value "8"
  #マクロで文字挿入
  function keyboard:string/input with storage keyboard: argument

#キー色変更
  #block_state変更
  data modify entity @n[type=block_display,tag=keyboard_8] block_state.Name set value "gray_concrete"
  #5tickで元のblock_stateに戻す
  scoreboard players set @n[type=block_display,tag=keyboard_8] keyboard_tick_push 5
